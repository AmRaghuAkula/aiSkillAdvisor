...
**Handoff author:** Claude session working on HireAstra design
...
This document captures the **state of work** for the aiSkillAdvisor project as of 2026-06-01. The v0 of the advisor was designed and validated inside a HireAstra Claude Code session over ~2 hours of focused TDD work. That v0 is now being dogfooded on real HireAstra rebrand work. This repo is the **extraction point** for the standalone product.
...
| HireAstra dogfooding | 🟡 In active use; logging refinements |
| Standalone product extraction | ⏳ Not started — waiting for 3+ pilot validations |
| First real pilot (REBRAND-024 /signin) | 🟡 In progress in a parallel session |
| Repository | ✅ Public, MIT-licensed, README + handoff docs in place |
| Roadmap | ✅ Captured in PRODUCT_VISION.md + BACKLOG.md |

**Don't start v1 extraction yet.** Per the founder's explicit instruction (2026-06-01), the standalone product gets built only after 2-3 successful HireAstra pilots validate the discipline in real work. Extracting prematurely risks shipping an unvalidated product.
...
### 3. Two-section HireAstra-specific routing

The v0 instance on HireAstra has two sections:
...
These sections are HireAstra-specific. In the standalone product, sections become **configurable per-project profiles**.
...
|---|---|---|
| HireAstra v0 dogfooding | HireAstra design session | 🟡 Ongoing — every refinement logged in `reference/v0-improvisations.md` |
| REBRAND-024 /signin pilot | Parallel pilot session | 🟡 First real-work validation — should produce first pilot data row |
...
Run 2-3 real HireAstra pilots with the v0 active. Each pilot:
...
Pull HireAstra-specific bits out of the algorithm:
...
| **Don't touch the live HireAstra v0 memory file from this repo** | The file at `C:\Users\raghu\.claude\projects\....\memory\feedback_proactive_skill_advisor.md` is the LIVE instance. This repo holds COPIES for handoff context. Edits to v0 happen in the memory directory; this repo logs them after the fact. |
...
| [`reference/v0-improvisations.md`](../reference/v0-improvisations.md) | Change log from HireAstra dogfooding (the v1 changelog) |
...
The aiSkillAdvisor repo holds documentation copies. The **live, mutable** v0 lives in the HireAstra session's memory directory:

```
SOURCE OF TRUTH (do not edit from this repo):
  C:\Users\raghu\.claude\projects\c--Users-raghu-Projects-OnePersonAICompany\memory\
...
When refinements happen during HireAstra work:
