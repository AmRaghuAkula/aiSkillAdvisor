 ...
 */
export function buildUserPromptSubmitOutput(signals) {
    const hint = signals.length > 0
        ? `Detected work-type hints: ${signals.join(", ")}. `
        : "";
    const context = `aiSkillAdvisor: ${hint}Check whether an installed skill (see the SessionStart ` +
